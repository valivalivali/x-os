...
#include "userspace/lib/xgfx/xgfx_font_terminus.h"
...
static inline void write_pixel_blend(xgfx_surface_t *s, int x, int y,
                                     uint32_t color, uint8_t alpha) {
    if (x < 0 || x >= s->w || y < 0 || y >= s->h) return;
    if (alpha == 0) return;
    uint32_t dst = s->pixels[y * s->stride + x];
    ...
    uint16_t inv = 256 - alpha;
    uint8_t r = (uint8_t)((sr * alpha + dr * inv) >> 8);
    uint8_t g = (uint8_t)((sg * alpha + dg * inv) >> 8);
    uint8_t b = (uint8_t)((sb * alpha + db * inv) >> 8);
    uint8_t a = (uint8_t)0xFF;
    if ((dst >> 24) < 0xFF) a = (uint8_t)((dst >> 24) + alpha);
    if (a > 0xFF) a = 0xFF;
    s->pixels[y * s->stride + x] = ((uint32_t)a << 24) | (r << 16) | (g << 8) | b;
}
...
/* ---- Text: Terminus 8x16 font (from FreeBSD vt_font_default.c) -------- */
/* Uses xgfx_font_lookup() for full Unicode support (2331 glyphs). */

void xgfx_draw_text(xgfx_surface_t *s, int x, int y, const char *str, uint32_t color) {
    int cx = x;
    for (const char *p = str; *p; p++) {
        const uint8_t *row = xgfx_font_lookup((uint32_t)(unsigned char)*p);
        for (int r = 0; r < 16; r++) {
            uint8_t bits = row[r];
            for (int c = 0; c < 8; c++) {
                if (bits & (0x80 >> c))
...
/* Anti-aliased text: edge feathering on the bitmap font.
 * For each "on" pixel, count how many of its 4 neighbors are also "on".
 * Interior pixels get full alpha; edge/corner pixels get reduced alpha
 * for a softening effect. 5 alpha levels: 255, 220, 180, 140, 100. */
void xgfx_draw_text_aa(xgfx_surface_t *s, float x, float y, const char *str, uint32_t color) {
    static const uint8_t edge_alpha[5] = {100, 140, 180, 220, 255};

    float cx = x;
    for (const char *p = str; *p; p++) {
        const uint8_t *glyph = xgfx_font_lookup((uint32_t)(unsigned char)*p);
        int ix = (int)cx;
        int iy = (int)y;
        ...
        for (int r = 0; r < 16; r++) {
            ...
            for (int c = 0; c < 8; c++) {
                ...
                /* Count on-neighbors (up/down/left/right) */
                int n = 0;
                if (r > 0  && (glyph[r - 1] & (0x80 >> c))) n++;
                if (r < 15 && (glyph[r + 1] & (0x80 >> c))) n++;
                if (c > 0  && (bits & (0x80 >> (c - 1))))   n++;
                if (c < 7  && (bits & (0x80 >> (c + 1))))   n++;

                uint8_t alpha = edge_alpha[n];
                write_pixel_blend(s, ix + c, iy + r, color, alpha);
            }
        }
        cx += 8.0f;
    }
